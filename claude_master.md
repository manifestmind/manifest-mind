...
4. **SHA → Firebase** : sur la page *Signature de l'application*, copier **les DEUX certificats** (clé de signature de l'app + clé d'importation), chacun **SHA-1 + SHA-256**, → Firebase → *Paramètres du projet* → *Général* → app Android `com.manifestmind.app` → *Ajouter une empreinte*. Prend effet quasi immédiat, **pas de rebuild**. L'empreinte « clé de signature de l'app » n'apparaît qu'**après** l'upload (étape 3).
   - ⚠️ **CHEMIN PLAY CONSOLE (déplacé par Google, 2026-07) :** les empreintes de signature sont sous **« Protégé avec Play » → « Protection Play Store » → « accéder à la signature d'application Play »**. La page *Vue d'ensemble de l'intégrité* n'affiche **PAS** les empreintes (piège qui fait tourner en rond).
   - 🔴 **APRÈS avoir ajouté les SHA dans Firebase : RETÉLÉCHARGER `google-services.json` → REMPLACER le fichier local (racine du repo) → COMMITTER, AVANT tout build.** `app.json` pointe `android.googleServicesFile: "./google-services.json"` (fichier LOCAL committé) et EAS n'en récupère AUCUNE version distante → sans ce remplacement, le build embarquerait l'ancienne config et Google Sign-In casserait sur les installs Play. Fait le 2026-07-30 (commit du fichier avec le client OAuth de la clé de signature Google). NB : `google-services.json` ne stocke que les **SHA-1** (comme `certificate_hash`) ; les SHA-256 vivent côté Firebase (App Check), leur absence du fichier est **normale**.
...
## 🧠 ENSEIGNEMENTS RÉVOCATION ADAPTY (2026-07-30) — À NE JAMAIS RÉOUBLIER (commit `8798ebd`)
Découverts au 2ᵉ test téléphone : l'abonnement expirait mais `subscription_active` restait `true`. Trois vérités durables :

1. **🔴 Adapty n'émet PAS `access_level_updated` à l'EXPIRATION — par CONCEPTION.** Doc à l'appui : *« Adapty doesn't send access_level_updated upon subscription expiration — please refer to the expires_at value to end the subscriptions on your side. »* ([Adapty Webhooks](https://adapty.io/docs/webhook)). `access_level_updated` part sur les octrois/changements actifs, pas sur la perte passive d'accès par écoulement du temps. **Conséquence : on ne peut PAS en faire l'unique source de révocation.** → On révoque désormais sur les événements de PERTE explicites : `subscription_expired` / `subscription_refunded` / `trial_expired` / `non_subscription_purchase_refunded`. Preuve du bug : séquence réelle = `subscription_started, access_level_updated, subscription_renewal_cancelled, access_level_updated, subscription_expired` — **aucun `access_level_updated` après l'expiration**.

2. **🔴 `profile_has_access_level` est un FAUX AMI — NE JAMAIS l'utiliser comme discriminant de révocation.** Il *paraît* parfait (« état net tous produits confondus »), mais **preuve empirique** : dans l'événement `subscription_expired` réel, `profile_has_access_level = TRUE` alors que l'utilisateur n'avait QUE le mensuel qui venait d'expirer (état net réel = false). Il vaut donc **`true` à l'instant précis où on a besoin de `false`** (effet de bord de bordure : `expires_at = event_datetime`, l'accès n'est pas encore recalculé). Si on écrivait `subscription_active = profile_has_access_level`, on recréerait le bug. **Écarté sur les DONNÉES, pas sur la doc** (qui, elle, le présentait à tort comme fiable).
...
## 🔐 NOTE SÉCURITÉ (2026-08-08) — RESTRICTION DES CLÉS API À TRAITER À FROID
- **Point à traiter, PAS urgent** : appliquer les **restrictions de clés API Firebase** (Google Cloud Console). **À froid, APRÈS le test fermé** — ne rien faire pendant que les 25 testeurs sont en v6.
- **Raison (nouvelle depuis ce jour)** : le dépôt GitHub `manifestmind/manifest-mind` est **PUBLIC** (vérifié via l'API GitHub), et il contient désormais **DEUX fichiers de configuration cliente Firebase committés** : `google-services.json` (Android, déjà présent) **et** `GoogleService-Info.plist` (iOS, committé ce jour au commit `3552f3a` pour débloquer le build EAS iOS). S'ajoute la clé web `EXPO_PUBLIC_FIREBASE_API_KEY` déjà dans le bundle web.
- **Ce que ça N'EST PAS** : ce ne sont **pas** des secrets. Clés API **clientes** Firebase = identifiants publics, extractibles de n'importe quelle app installée. La **sécurité des données** vient des **règles Firestore** (déjà en place, point A.1) — **inchangée** par le fait que ces fichiers soient publics.
...
## 💱 PRIX iOS — ÉCART STOREFRONT — ✅ CLOS le 2026-08-14 (AUCUN CORRECTIF — NE PAS ROUVRIR)

> **🔒 SUJET CLOS. Ne pas rouvrir, ne pas « corriger », ne pas re-tester.** La vérification exigée par la règle du 2026-08-09 (ci-dessous) A ÉTÉ FAITE après activation des produits Apple. Verdict : **comportement NORMAL, aucun défaut, aucun correctif à faire.** La section historique qui suit est conservée pour mémoire uniquement — sa « RÈGLE » est DÉSARMÉE.
>
> **Ce qui a été constaté (2026-08-14, produits Apple TOUS ACTIFS)** — prix Apple en vigueur : **France 8,49 €** · **Panama 7,99 $** · **Canada 9,99 $**. Sur l'iPhone de l'utilisatrice : paywall **7,99 $**, feuille de paiement Apple **8,49 €**. Soit **le prix panaméen d'un côté, le prix français de l'autre — deux prix JUSTES, pour deux pays différents.**
>
> **Mécanisme (définitif)** : **Adapty résout le pays par l'ADRESSE RÉSEAU** (l'utilisatrice vit au Panama) ; **Apple/StoreKit facture selon le PAYS DU COMPTE App Store** (français). L'écart n'apparaît donc QUE si pays réseau ≠ pays du compte Apple. Cas d'une expatriée / d'un VPN — **pas le cas d'une utilisatrice normale**.
>
> **❌ CE QUE CE N'EST PAS — deux fausses pistes définitivement écartées :**
> 1. **PAS un écart hors taxes / taxes comprises.** En Europe, le prix saisi dans App Store Connect **est déjà le prix TTC payé par le client** : `localizedString` sur un iPhone FR **est** le montant de la feuille Apple. **Une utilisatrice française sur un iPhone français voit le même montant des deux côtés.** (Les territoires « taxe ajoutée » — US, Canada — sont les seuls où le store ajoute la taxe à l'encaissement ; aucune app ne peut y remédier, StoreKit n'expose pas le montant de taxe.)
> 2. **PAS un mauvais champ lu dans le code.** Vérifié dans les types du SDK (`node_modules/@adapty/core/dist/index.d.mts:483-506`) : `AdaptyPrice` n'a QUE 4 champs — `amount`, `currencyCode`, `currencySymbol`, `localizedString`. **Aucun champ « taxes comprises » n'existe** (zéro occurrence de `tax` sur toute la surface de types). `services/purchasesNative.ts:159` lit `localizedString`, **le seul champ formaté disponible** : il n'y a pas d'alternative à lire.
>
> **🚫 NE PAS AJOUTER de mention « hors taxes » sous les prix** : ce serait **FAUX en France** (prix déjà TTC) et faux sur Android FR. La pratique standard iOS = afficher `displayPrice`/`localizedString` tel quel, sans mention. C'est déjà ce que fait le code.
>
> **✅ ANDROID — aucun écart, garanti par Google.** La France figure sur la liste officielle des pays à **tarification taxe incluse** de Google Play : « les prix affichés aux acheteurs doivent être égaux au montant payé au moment du paiement ». Les prix saisis HT dans la Play Console sont convertis en TTC par Google, et c'est ce TTC qui remonte dans `formattedPrice` → `localizedString`. **Les 25 testeurs ne voient aucun écart.** Rien à faire côté Android non plus.
>
> **Note de portée** : les montants **129,99 $ / 79,99 $ / 11,99 $** cités dans la section historique ci-dessous étaient les **ANCIENS prix**, changés depuis. Ne pas les rapprocher des prix actuels.

### 📜 Historique (2026-08-09) — conservé pour mémoire, RÈGLE DÉSARMÉE
- **Symptôme** : sur iPhone FR (TestFlight/sandbox), le paywall natif a affiché **129,99 $ / 79,99 $ / 11,99 $ (USD)**, alors que la feuille de paiement Apple s'ouvrait à **13,99 € (EUR)** pour le mensuel. Affiché ≠ facturé.
- **Ce que ce N'EST PAS** : PAS un prix codé en dur. Ces montants **n'ont JAMAIS existé dans le code** (vérifié sur TOUT l'historique git). La constante `PRICES` vaut 149/12,99/79. Et PAS un « repli natif » : sur natif il n'y a **pas** de repli codé en dur.
- **Ce que c'est** : Adapty a bien **renvoyé les 3 produits AVEC un prix** (`res.ok:true`), mais **résolus sur le storefront US**, alors que StoreKit facture sur le storefront **FR** de l'appareil (mêmes produits, paliers de prix par pays ; l'EUR inclut la TVA → plus élevé). Cause probable : **état brouillon** → Adapty n'a pas le prix localisé live et retombe sur un prix serveur/base (USD).
- **🔑 LA DISTINCTION QUI NOUS A FAIT TOURNER EN ROND** : le tout-ou-rien de `useLocalizedPrices` (natif Adapty → `…` + achat bloqué + « Prix indisponibles ») protège contre **« Adapty ne renvoie RIEN »** (`res.ok:false`), **PAS** contre **« Adapty renvoie le prix du MAUVAIS PAYS »** (`res.ok:true` avec un prix d'un autre storefront). Ne plus confondre les deux.
- **Piste diagnostic (dashboard Adapty)** : les événements portent `store_country` et `profile_country` — observés DIVERGENTS chez une testeuse (store TH / profile AE). Si Adapty se fie à l'un de ces champs (ou à un défaut) plutôt qu'au storefront réel de l'appareil, c'est l'explication. Vérifier le `store_country` résolu du profil + si le prix Apple est « synced from App Store » ou fallback.
...
- ~~**🔴 RÈGLE — NE PAS refaire « Option A » sans ce test** : après **approbation** des produits, RE-VÉRIFIER sur un **compte FR** que **prix affiché == prix feuille Apple**. **SI l'écart PERSISTE** → **ALORS** un correctif client devient nécessaire ET justifié.~~ → **⛔ RÈGLE ÉTEINTE le 2026-08-14.** Le test a été fait (produits actifs) : l'écart s'explique intégralement par **pays réseau (PA) ≠ pays du compte Apple (FR)**, les deux prix étant justes. **La condition « correctif nécessaire » n'est PAS remplie et ne doit pas être invoquée.** Ne pas rejouer ce test, ne pas repartir sur « Option A ».
...
## 🔴 RÈGLE — REVÉRIFIER LE MANIFESTE DE CONFIDENTIALITÉ ADAPTY À CHAQUE MONTÉE DE VERSION SDK (2026-08-12)
- **Pourquoi** : la déclaration App Store « App Privacy » doit correspondre à ce que le SDK collecte RÉELLEMENT. Une version plus récente d'Adapty pourrait déclarer des **types de données supplémentaires** → la déclaration deviendrait FAUSSE sans que personne ne s'en aperçoive (risque de retrait Apple).
- **MÉTHODE (impérative)** : aller lire le fichier **`PrivacyInfo.xcprivacy` du TAG de la version EXACTE installée**, PAS du master :
  `https://github.com/adaptyteam/AdaptySDK-iOS/blob/<VERSION>/Sources/PrivacyInfo.xcprivacy` (ex. tag `4.0.1`). La version iOS est épinglée par `react-native-adapty` (SPM `exactVersion`).
...
## 🧹 DETTE — VALEURS MORTES À TRAITER À FROID (relevé 2026-08-15) — NE PAS MÉLANGER À UN COMMIT FONCTIONNEL
- **Règle de méthode (décision utilisatrice, 2026-08-15)** : ces nettoyages se font **à froid, dans un commit dédié**, JAMAIS glissés dans une amélioration fonctionnelle. Motif : c'est exactement ce type de valeur morte (prix codés en dur) qui a fait perdre du temps la semaine du 11 août — une valeur périmée qui traîne finit par être lue comme la vérité.
- **Clés i18n orphelines — `visionBoard.valider` et `visionBoard.passer`** (relevées pendant le Bloc 2 v8) : présentes dans les **3 langues** (`translations.ts`, FR ~417 / EN ~1017 / ES ~1603), **appelées NULLE PART** dans `vision-board.tsx` (grep : zéro correspondance).
  - **Origine** : vestiges d'un ancien **bouton de validation** (« Valider mon Vision Board · +5 pts » / « Passer cette étape sans points »). Les 5 points sont désormais attribués **automatiquement au focus de l'écran** (`vision-board.tsx:97-114`, `useFocusEffect`) — le bouton a disparu, les libellés sont restés.
  - **Piège de lecture** : ces clés font croire à un parcours « l'utilisatrice valide son vision board », qui **n'existe plus**. Ne pas s'en servir pour raisonner sur le comportement réel.
  - **NON supprimées volontairement** le 2026-08-15 (hors périmètre du Bloc 2).
- **Autres restes à regrouper le jour du nettoyage** : commentaires évoquant l'ancien prestataire de paiement · repli USD statique `PRICES`/`formatUSD` (déjà supprimé côté web par `1cf6841`, vérifier qu'il ne reste rien) · cf. aussi la section « 🧹 PERMISSIONS ANDROID — sources & nettoyage » (analyse 2026-08-01, non traitée).
- **Clé i18n orpheline — `pricing.plans.annuel.unite` (« /mois », « /mo », « /mes »)**, ajoutée à la liste le **2026-08-22**. Devenue inutilisée quand la carte annuelle a été inversée (`4c197f4`) : la colonne de prix affiche désormais **toujours** `uniteAn` (« /an »), la ternaire qui choisissait entre les deux unités a disparu. `uniteAn` reste, lui, bien utilisé sur les 2 paywalls.
  - **Volontairement NON supprimée dans le commit fonctionnel**, en application de la règle de méthode ci-dessus. Un nettoyage glissé dans une correction rend le diff illisible au moment précis où il doit être relu de près — ici, un écran de revenus sur trois plateformes.
- **Montant CODÉ EN DUR — `pricing.bottomText`** : « Moins de 0,50 $ pour changer ta vie » (et ses équivalents EN/ES). Repéré le **2026-08-22**.
  - **Jamais affiché en production** : sa seule branche est `pricesPhase === 'web'`, qui correspond au mode INERTE (`canPay()` faux). Or `PADDLE_ACTIVE` et `STORES_ACTIVE` valent tous deux `true` → cette branche n'est atteinte ni sur le site ni sur mobile.
  - **Pourquoi le noter quand même** : c'est un montant en dur, dans une devise fixe, non localisé, qui survivrait à un changement de prix sans que personne ne s'en aperçoive. Exactement la classe de valeur morte qui a coûté du temps mi-août. **Si cette branche redevenait atteignable un jour, ce serait aussi un prix dérivé de plus** — donc une prise supplémentaire au regard d'Apple 3.1.2(c) et de la politique Play.

## 📮 CHANTIER REPORTÉ — RETRAIT DU CONSENTEMENT MARKETING DEPUIS LES PARAMÈTRES (noté 2026-08-15)
- **Livré en v8** (commit du Bloc 3) : la COLLECTE du consentement — case non cochée par défaut sur `pricing.tsx` + `pricing-upgrade.tsx` (branche web) et `activation.tsx` (chemin natif), écriture dans `marketing_consents/{uid}`, suppression RGPD branchée dans `parametres.tsx`.
- **REPORTÉ volontairement** : l'interrupteur permettant de RETIRER son consentement depuis les Paramètres. Motif du report (décision utilisatrice 2026-08-15) : la v8 était déjà le plus lourd des trois blocs, à 4 jours de la fin du test fermé.
- **🔑 RAISON TECHNIQUE À RETENIR** : c'est **le même mécanisme d'écriture et la même collection** que la collecte (`services/marketingConsent.ts`, `marketing_consents/{uid}`). Le faire plus tard **rouvrira `parametres.tsx` une deuxième fois** — c'était le seul argument pour le faire tout de suite. À regrouper avec le prochain chantier qui touche cet écran.
- **Ce que le report NE met PAS en danger** : le lien de désinscription obligatoire dans chaque e-mail (RGPD art. 21, art. L34-5 CPCE) est fourni par la **plateforme d'emailing**, pas par l'app. **Cette obligation s'attache au PREMIER ENVOI, pas à la v8.** Aucune plateforme choisie à ce jour (Zoho Mail = messagerie, pas outil d'envoi en nombre) → rien d'exigible tant qu'aucun e-mail marketing n'est parti.
- **⚠️ À FAIRE AVANT LE PREMIER ENVOI** : s'assurer que la plateforme retenue insère bien le lien de désinscription, et que le retrait y est effectif. C'est le vrai jalon, pas la v8.

## 🌐 WEB PROD REDÉPLOYÉE (2026-08-15) — VERSION 8 : retour auth · phrase vision board · consentement RGPD · correctif paywall
- **Contenu** (6 commits, du plus ancien au plus récent) : `22c38c2` lien « ← Retour » sur `auth.tsx` (seul écran sans sortie) · `e5b5986` phrase fixe du vision board · `8b6a115` note de dette (clés i18n orphelines) · `3f1d325` consentement marketing RGPD · `36c5c95` note interrupteur reporté · `fdbfed9` correctif du bouton d'achat qui recouvrait la liste des avantages.
- **Déploiement en TROIS temps, dans cet ordre (à refaire tel quel)** :
  1. `npx firebase deploy --only firestore:rules` — **AVANT** le site. L'inverse aurait ouvert une fenêtre où la case de consentement s'affiche alors que chaque écriture échoue en silence → consentements réellement donnés, et perdus sans trace.
  2. `npx expo export --platform web` (reconstruit `dist`, ne déploie rien).
...
## 🔇 À CREUSER À FROID — le son ne démarre pas sur NAVIGATEUR MOBILE ANDROID (repéré 2026-08-15, AUCUNE URGENCE)
- **Symptôme** : sur le SITE ouvert dans un navigateur mobile Android, la musique de séance ne démarre pas. Fonctionne sur ordinateur et dans l'application NATIVE.
- **⚠️ NE PAS RE-DÉCOUVRIR LA RÈGLE D'AUTOPLAY** : `BackgroundMusicProvider.tsx:97-122` la gère **DÉJÀ**. Sur web, aucune source n'est chargée au montage ; un écouteur (`pointerdown`/`touchstart`/`keydown`/`click`) attend le 1er geste, puis fait `replace(AUDIO_SOURCE)` → `play()`. Le diagnostic « il faut une interaction » est donc **déjà appliqué** — ce n'est pas la piste.
- **PISTE 1 (la plus probable) — l'écouteur est retiré même quand le démarrage ÉCHOUE.** `start()` (`:104-114`) appelle `remove()` **inconditionnellement**, et les écouteurs sont posés en `{ once: true }`. Si la 1ʳᵉ tentative échoue, il n'y a **JAMAIS de seconde chance** de toute la session. Sur ordinateur la 1ʳᵉ tentative réussit → invisible. Sur Android elle échouerait une fois, définitivement.
...
## ⭐ POUR L'APRÈS-LANCEMENT — notation intégrée à un MOMENT NATUREL (noté 2026-08-16, PAS pour maintenant)
- **Livré en v9** : une entrée « Noter l'application » dans les Paramètres, en **LIEN DIRECT** vers la fiche du magasin (`parametres.tsx`, constantes `STORE_REVIEW_URL` / `STORE_REVIEW_FALLBACK`).
- **🔑 POURQUOI PAS L'API INTÉGRÉE DERRIÈRE CE BOUTON — ne pas revenir là-dessus** : Apple ET Google l'interdisent explicitement dans ce cas d'usage. Apple : `SKStoreReviewController` ne doit « jamais être appelé en réponse à une action de l'utilisateur, comme l'appui sur un bouton » (plafond système : 3 affichages/an/utilisateur). Google, littéralement : « Vous ne devez pas avoir de bouton déclenchant l'API […] **pour ce cas d'usage, redirigez plutôt vers le Play Store.** » Un bouton derrière une interface à quota est mort une fois sur deux, sans signal.
- **CE QU'IL RESTERAIT À FAIRE, APRÈS LE LANCEMENT** : ajouter l'interface intégrée (`expo-store-review`) déclenchée à un **MOMENT NATUREL** — typiquement **à la fin d'un cycle réussi**, sur `celebration.tsx` — **EN PLUS** du lien des Paramètres, qui reste. C'est cette combinaison que les deux magasins recommandent, et **c'est elle qui fait réellement monter les notes** : l'utilisatrice est sollicitée au moment où elle vient de réussir quelque chose, sans quitter l'app.
- **POURQUOI C'EST REPORTÉ, et pas un oubli** : `expo-store-review` est un **module natif**. Il impose (a) une modification de `package.json`, donc la perte de l'invariant « manifeste identique » qui sert de preuve à chaque build, et (b) **DEUX reconstructions**, Android ET iOS. Or au 2026-08-16 la soumission iOS est en examen chez Apple, et la v9 est la **DERNIÈRE** des 3 mises à jour du test fermé.
- **Conditions pour le sortir du tiroir** : lancement effectué · plus de contrainte sur le nombre de mises à jour · une reconstruction des deux plateformes déjà prévue pour autre chose (à regrouper avec, jamais pour lui seul).
- **Garde-fou à ne pas oublier le jour venu** : ne JAMAIS déclencher l'interface après un échec ou un abandon, ne jamais l'accompagner d'une incitation à bien noter (motif de sanction chez les deux magasins), et espacer les sollicitations.

## 🍎 À VÉRIFIER APRÈS LA PUBLICATION iOS — la fiche App Store doit exister, sinon le bouton « Noter » mène dans le vide
- **Contexte (2026-08-16)** : la v9 ajoute une entrée « Noter l'application » dans les Paramètres. Sur iOS elle ouvre `itms-apps://apps.apple.com/app/id6799153444?action=write-review` (constante `STORE_REVIEW_URL`, `parametres.tsx`).
- **L'identifiant `6799153444` est CONFIRMÉ BON** — visible partout dans App Store Connect. Testé le 2026-08-16 : `https://apps.apple.com/app/id6799153444` **ne trouve aucune page**. ⚠️ **CE N'EST PAS UNE COQUILLE** : l'app est en examen et la publication est réglée en MANUELLE, donc la fiche publique n'existe pas encore. Ne PAS « corriger » l'identifiant sur la foi de ce test.
- **🔴 À FAIRE LE JOUR DE LA PUBLICATION iOS, avant de considérer le sujet clos** : ouvrir `https://apps.apple.com/app/id6799153444` dans un navigateur et **confirmer que la fiche s'affiche**.

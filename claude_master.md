...
19. Nettoyer **code mort** (logs non `__DEV__` dans `useShare.ts`/`auth.tsx`/`paddle.ts`, `icon-symbol`, dépendance `expo-av`, assets template `react-logo*`, mockups HTML racine, `handleRestore` vides). 🚨 **EXCEPTION `paddle.ts`** : les `console.error`/`console.warn` des échecs de checkout (point 9) sont des **diagnostics INTENTIONNELS à CONSERVER** — ne retirer QUE d'éventuels logs debug non liés au paiement. 📎 Aussi repéré (2026-07-16, non bloquant) : `ImagePicker.MediaTypeOptions` est **déprécié** (warning TS dans `vision-board.tsx` l.~153 et `profil.tsx` l.~170) → migrer vers la nouvelle API `mediaTypes: ['images']` pendant ce nettoyage, en re-testant le pick photo web derrière.
